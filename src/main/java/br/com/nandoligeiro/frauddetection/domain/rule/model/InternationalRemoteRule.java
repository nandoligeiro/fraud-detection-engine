...
public final class InternationalRemoteRule implements FraudRule {
    ...
    public InternationalRemoteRule(int version, int priority, boolean enabled, FraudSeverity severity, String homeCountry) {
        if (version <= 0) {
    ...
    public RuleEvaluationResult evaluate(Transaction transaction) {
        ...
        boolean triggered = enabled && transaction.isCardNotPresent() && transaction.isInternationalComparedTo(homeCountry);
        if (!triggered) {
            return RuleEvaluationResult.notTriggered(id(), version, severity);
        }
        return RuleEvaluationResult.triggered(id(), version, severity, "International card-not-present transaction detected");
    }
}
